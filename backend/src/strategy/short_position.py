"""Short position tracking for strategy management"""
...
class ShortPosition:
    """Tracks individual short positions for accurate P&L calculation"""
    
    def __init__(self, usd_amount: Decimal, entry_price: Decimal, 
                 eth_amount: Decimal, unit_opened: int):
        self.usd_amount = usd_amount      # Original USD shorted
        self.entry_price = entry_price     # Entry price when short opened
        self.eth_amount = eth_amount       # ETH amount shorted
        self.unit_opened = unit_opened     # Unit level when opened
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ShortPosition':
        ...
        if data.get('timestamp'):
            short.timestamp = datetime.fromisoformat(data['timestamp'])
        return short